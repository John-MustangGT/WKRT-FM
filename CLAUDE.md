...
| `wkrt/tts.py` | Dispatches to Piper, Google Cloud TTS, or Kokoro per `dj_cfg`; converts WAV→MP3; caches by SHA-256 of `(voice_id, text)` |
...
- **`kokoro`** — Kokoro ONNX neural TTS; requires `pip install kokoro-onnx soundfile` and model files. Download via `INSTALL_KOKORO=1 bash scripts/setup_voices.sh` (installs v1.0 by default: `voices/kokoro-v1.0.onnx` + `voices/kokoro-voices-v1.0.bin`). No cloud account or external binary needed—runs entirely on-device. ONNX model instance is cached in memory after first load. Pronunciation substitutions are applied (same table as Piper). Falls back to silence if files are missing. Good choice for Raspberry Pi or bandwidth-constrained deployments.
...
ICY `StreamTitle` and other metadata sent to Icecast are now template-driven. Configure global defaults in `[metadata_mapping]` and override per-target:

```toml
[metadata_mapping]
StreamTitle = "%a - %T"          # artist - title (global default)
...
Comment = "WKRT-FM 104.7 Boston"  # override the global default
...
- `%L` = Label / record company (from annotation)

Metadata resolution is handled by `wkrt/config.py`: `resolve_metadata_template()` replaces placeholders, and `get_metadata_for_target()` merges global defaults with per-target overrides.
...
The engine also polls `http://<host>:<port>/status-json.xsl` every 15 seconds to reconcile listener count independently of webhooks. ICY metadata (including `StreamTitle` resolved from `[metadata_mapping]`) is pushed on every track change and at the moment the DJ talkover begins.
